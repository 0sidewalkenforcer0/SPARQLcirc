...
#
# The factored plan names its private message relations after a per-run UUID, so that one
...
queries() {
  ls "$ROOT"/reference/paper/queries/watdiv/10M/*/*.rq
  find "$ROOT/reference/tpch" -name '*.rq' | sort | head -6
}

current() {
  for q in $(queries); do
    for mode in flat factored; do
      ...
      java -cp "$JAR" npcs.circuit.CircuitRun --construction=$mode Standard \
        ...
        | sed 's/urn:sc:msg:[0-9a-f]\{64\}:/urn:sc:msg:WORKSPACE:/g'
    done
  done
}
