...
public final class Filters {
    ...
    /**
     * One conjunct that the physical factorised planner may place independently.  The flat
     * rewriting keeps the user's original FILTER expression intact; splitting a conjunction here is
     * only a physical-plan optimization.  A solution survives {@code FILTER(a && b)} exactly when it
     * survives {@code FILTER(a) FILTER(b)}, including SPARQL expression errors, so the split does not
     * change the selected mappings.
     */
    static final class Condition {
        private final String expression;
        private final List<String> variables;

        Condition(String expression, Set<String> variables) {
        ...
        List<String> variables() { return variables; }
        ...
        @Override public boolean equals(Object other) {
            ...
            return expression.equals(condition.expression)
                    && variables.equals(condition.variables);
        }

        @Override public int hashCode() { return Objects.hash(expression, variables); }
    }
     ...
     */
    public static List<String> of(TupleExpr operand) {
        List<Filter> found = filters(operand);
    ...
    /**
     * Canonical FILTER conjuncts with their variable dependencies.  The factorised planner uses this
     * metadata to put a local condition in a base scan and a cross-relation condition in the first
     * join/marginalization pass whose row binds every referenced variable.
     */
    static List<Condition> conjuncts(TupleExpr operand) {
        List<Condition> out = new ArrayList<>();
        for (Filter filter : filters(operand)) {
            validateScope(filter);
            List<ValueExpr> conjuncts = new ArrayList<>();
            flattenConjunction(filter.getCondition(), conjuncts);
            for (ValueExpr conjunct : conjuncts) {
                out.add(new Condition(render(conjunct), conditionVars(conjunct)));
            }
        }
        out.sort(Comparator.comparing(Condition::expression)
                .thenComparing(condition -> String.join("\u001f", condition.variables())));
        return out;
    }

    private static List<Filter> filters(TupleExpr operand) {
    ...
    private static void validateScope(Filter filter) {
        Set<String> scope = patternVars(filter.getArg());
        Set<String> used = conditionVars(filter.getCondition());
    ...
    private static void flattenConjunction(ValueExpr expression, List<ValueExpr> out) {
        if (expression instanceof And) {
            And and = (And) expression;
            flattenConjunction(and.getLeftArg(), out);
            flattenConjunction(and.getRightArg(), out);
        } else {
            out.add(expression);
        }
    }
    ...
    public static String emit(List<String> conditions) {
        StringBuilder sb = new StringBuilder();
        for (String c : conditions) sb.append("\tFILTER(").append(c).append(") \n");
        return sb.toString();
    }

    /** SPARQL text for physical-plan conjuncts. */
    static String emitConditions(List<Condition> conditions) {
        StringBuilder sb = new StringBuilder();
        for (Condition condition : conditions) {
            sb.append("\tFILTER(").append(condition.expression()).append(") \n");
        }
        return sb.toString();
    }

    /** Variables bound by the triple patterns of a subtree (the only binders in our fragment). */
    public static Set<String> patternVars(TupleExpr te) {
    ...
    public static Set<String> conditionVars(ValueExpr condition) {
        LinkedHashSet<String> out = new LinkedHashSet<>();
     ...
     * than approximated.
     */
    public static String render(ValueExpr e) {
