...
public class CircuitFilterTest {
    ...
    @Test
    public void filteredBgpUsesFactorisedPlacement() {
        Repository repo = new SailRepository(new MemoryStore());
        try (RepositoryConnection con = repo.getConnection()) {
            ...
            String query = "SELECT ?z WHERE { <urn:s> <urn:p> ?m . ?m <urn:q> ?z "
                    + "FILTER(?z != <urn:drop>) }";
            CircuitConstructionPlan factorised = new CircuitRewriter(
                    Reification.STANDARD, ConstructionMode.FACTORISED, "junit-filter-factorised")
                    .constructionPlan(query);
            assertEquals(ConstructionMode.FACTORISED, factorised.effectiveMode());
            assertNull(factorised.fallbackReason());
            assertTrue(factorised.requiresFeedback());
            for (String construct : factorised.queries()) {
                new SPARQLParser().parseQuery(construct, null);
            }

            Model viaFactorised = executePlan(con, query, ConstructionMode.FACTORISED);
            Model viaFlat = executePlan(con, query, ConstructionMode.FLAT);
            assertEquals(answerRoots(viaFlat), answerRoots(viaFactorised));
            assertEquals(1, answerRoots(viaFlat).size());
            assertEquals(1, answerRoots(viaFactorised).size());
            assertFalse(leaves(viaFlat).contains("urn:r:3"));
            assertFalse(leaves(viaFactorised).contains("urn:r:3"));
        } finally {
            repo.shutDown();
        }
    }
    ...
    @Test
    public void factorisedCrossRelationFilterDoesNotRecombineRejectedMappings() {
        Repository repo = new SailRepository(new MemoryStore());
        try (RepositoryConnection con = repo.getConnection()) {
            reify(con, "urn:r:p-a", "urn:a", "urn:p", "urn:mid");
            reify(con, "urn:r:p-b", "urn:b", "urn:p", "urn:mid");
            reify(con, "urn:r:q-a", "urn:mid", "urn:q", "urn:a");
            reify(con, "urn:r:q-b", "urn:mid", "urn:q", "urn:b");

            String query = "SELECT ?x ?z WHERE { ?x <urn:p> ?m . ?m <urn:q> ?z "
                    + "FILTER(?x != ?z) }";
            CircuitConstructionPlan plan = new CircuitRewriter(
                    Reification.STANDARD, ConstructionMode.FACTORISED, "junit-filter-cross")
                    .constructionPlan(query);
            ...
            assertTrue("the cross-relation condition must be evaluated before ?m disappears",
                    plan.steps().stream().anyMatch(step -> step.label().contains("filter[1]")));

            Model flat = executePlan(con, query, ConstructionMode.FLAT, "junit-filter-cross");
            Model factorised = executePlan(
                    con, query, ConstructionMode.FACTORISED, "junit-filter-cross");
            assertEquals("only (a,b) and (b,a) survive", 2, answerRoots(factorised).size());
            assertEquals(answerRoots(flat), answerRoots(factorised));

            String[] tokens = {"urn:r:p-a", "urn:r:p-b", "urn:r:q-a", "urn:r:q-b"};
            for (int mask = 0; mask < (1 << tokens.length); mask++) {
                Set<String> world = new LinkedHashSet<>();
                for (int bit = 0; bit < tokens.length; bit++) {
                    if ((mask & (1 << bit)) != 0) world.add(tokens[bit]);
                }
                for (Resource root : answerRoots(flat)) {
                    assertEquals("flat/factorised Boolean semantics differ in world " + world,
                            CircuitTestSupport.evaluate(flat, root, world,
                                    new java.util.HashMap<Resource, Boolean>()),
                            CircuitTestSupport.evaluate(factorised, root, world,
                                    new java.util.HashMap<Resource, Boolean>()));
                }
            }
        } finally {
    ...
    public void outputBindingsExecuteAndKeepQueryIdentity() {
        ...
        try (RepositoryConnection con = repo.getConnection()) {
            ...
            reifyLiteral(con, "urn:r:m", "urn:s", "urn:m", 1);
            ...
            CircuitConstructionPlan direct = new CircuitRewriter(
                    Reification.STANDARD, ConstructionMode.FACTORISED, "junit-bind-factorised")
                    .constructionPlan(plus);
            assertEquals(ConstructionMode.FACTORISED, direct.effectiveMode());
            assertFalse(direct.requiresFeedback());
            assertTrue(direct.strategyFragments().contains("direct-terminal-bind"));

            String joined = "SELECT (?n + ?m AS ?x) WHERE { "
                    + "<urn:s> <urn:n> ?n . <urn:s> <urn:m> ?m "
                    + "BIND(?n * 100 AS ?unused) }";
            CircuitConstructionPlan factorised = new CircuitRewriter(
                    Reification.STANDARD, ConstructionMode.FACTORISED, "junit-bind-joined")
                    .constructionPlan(joined);
            assertEquals(ConstructionMode.FACTORISED, factorised.effectiveMode());
            assertTrue(factorised.requiresFeedback());
            assertNull(factorised.fallbackReason());
            assertTrue(factorised.strategyFragments().contains("terminal-bind-answer-sink"));
            String answerStep = factorised.steps().get(factorised.steps().size() - 1).query();
            assertTrue("the live projection BIND must run before the generated answer identity",
                    answerStep.indexOf("AS ?x") < answerStep.indexOf("urn:g:a:"));
            assertFalse("an unused BIND must not keep columns alive through the factorised plan",
                    answerStep.contains("AS ?unused"));

            Model plusCircuit = executePlan(con, plus);
            Model timesCircuit = executePlan(con, times);
            Model yearCircuit = executePlan(con, year);
            Model joinedCircuit = executePlan(
                    con, joined, ConstructionMode.FACTORISED, "junit-bind-joined-factorised");
            Model joinedFlat = executePlan(
                    con, joined, ConstructionMode.FLAT, "junit-bind-joined-flat");
            ...
            assertEquals(Collections.singleton(two), bindingValues(joinedCircuit, "x"));
            assertEquals(answerRoots(joinedFlat), answerRoots(joinedCircuit));
    ...
    @Test
    public void factorisedBindingsRetainDependenciesAndMergeDerivedAnswers() {
        Repository repo = new SailRepository(new MemoryStore());
        try (RepositoryConnection con = repo.getConnection()) {
            ValueFactory vf = con.getValueFactory();
            reify(con, "urn:r:pa", "urn:s", "urn:p", "urn:a");
            reify(con, "urn:r:pb", "urn:s", "urn:p", "urn:b");
            reifyValue(con, "urn:r:da", "urn:a", "urn:date",
                    vf.createLiteral("1995-03-18",
                            vf.createIRI("http://www.w3.org/2001/XMLSchema#date")));
            reifyValue(con, "urn:r:db", "urn:b", "urn:date",
                    vf.createLiteral("1995-11-02",
                            vf.createIRI("http://www.w3.org/2001/XMLSchema#date")));

            String query = "SELECT ?next WHERE { <urn:s> <urn:p> ?person . "
                    + "?person <urn:date> ?date "
                    + "BIND(YEAR(?date) AS ?year) BIND(?year + 1 AS ?next) }";
            CircuitConstructionPlan plan = new CircuitRewriter(
                    Reification.STANDARD, ConstructionMode.FACTORISED, "junit-bind-collapse-plan")
                    .constructionPlan(query);
            assertEquals(ConstructionMode.FACTORISED, plan.effectiveMode());
            assertTrue(plan.strategyFragments().contains("terminal-bind-answer-sink"));
            String sink = plan.steps().get(plan.steps().size() - 1).query();
            assertTrue("chained BIND order must be preserved",
                    sink.indexOf("AS ?year") < sink.indexOf("AS ?next"));

            Model factorised = executePlan(
                    con, query, ConstructionMode.FACTORISED, "junit-bind-collapse-factorised");
            Model flat = executePlan(
                    con, query, ConstructionMode.FLAT, "junit-bind-collapse-flat");
            assertEquals("both 1995 dates project to one answer", 1, answerRoots(factorised).size());
            assertEquals(answerRoots(flat), answerRoots(factorised));
            assertEquals(Collections.singleton(vf.createLiteral("1996",
                            vf.createIRI("http://www.w3.org/2001/XMLSchema#integer"))),
                    bindingValues(factorised, "next"));

            Resource root = answerRoots(factorised).iterator().next();
            assertTrue("the first derivation must feed the shared derived answer",
                    CircuitTestSupport.evaluate(factorised, root,
                            new HashSet<>(Arrays.asList("urn:r:pa", "urn:r:da")),
                            new java.util.HashMap<Resource, Boolean>()));
            assertTrue("the second derivation must feed the same shared derived answer",
                    CircuitTestSupport.evaluate(factorised, root,
                            new HashSet<>(Arrays.asList("urn:r:pb", "urn:r:db")),
                            new java.util.HashMap<Resource, Boolean>()));
            assertFalse("an incomplete derivation must not satisfy the answer",
                    CircuitTestSupport.evaluate(factorised, root,
                            new HashSet<>(Collections.singletonList("urn:r:pa")),
                            new java.util.HashMap<Resource, Boolean>()));
        } finally {
            repo.shutDown();
        }
    ...
    private static Model executePlan(RepositoryConnection con, String query, ConstructionMode mode) {
        return executePlan(con, query, mode, "junit-filter-" + mode.cliName());
    }
