...
_Generated by the overnight run 2026-07-11 01:40:14. Datasets/engines: WatDiv 10M & 100M on GraphDB 10.7.6, d4 v1, PostgreSQL 18 + ProvSQL 1.11. All numbers from this server; compare against the pre-registered predictions in `docs/EVALUATION.md`._


## E3 - construction scaling on an unmodified engine (GraphDB)

Bound (selective) S/L/F shapes; circuit built by the stock engine running our CONSTRUCT. `share` = T_string/T_circ (structural). **Legacy-column warning:** `plain_ms` is misnamed: `e3_run.py` obtains it by calling `get_npcs()`, so it is the NPCS per-answer provenance SELECT, not the original query. ROUND 9 adds the true B/R/N/C controls. *The pre-registered prediction was near-linear build ~ small const x original query; the existing ratio does not test that particular comparison.*
...
**Validated (match `docs/EVALUATION.md`):** E2 compactness (shallow 0.4–0.9×, deep-12x2 = 201.4×); E4
