...
REFERENCE_DIRECTORY = CONVERTER_PATH.parents[1]
...
E11_SPEC = importlib.util.spec_from_file_location(
    "e11_real", REFERENCE_DIRECTORY / "e11_real.py"
)
e11_real = importlib.util.module_from_spec(E11_SPEC)
E11_SPEC.loader.exec_module(e11_real)
...
class TpchRdfMappingTest(unittest.TestCase):
    ...
    def test_e11_loader_preserves_composite_row_identity(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            output, _ = self.convert_fixture(Path(temporary_directory))
            data, patterns = e11_real.load_tpch_q3(output)

        self.assertEqual(
            {
                "Customer/5": ("Customer/5", "c_mktsegment", "BUILDING"),
                "Order/6": ("Order/6", "o_cust", "Customer/5"),
                "LineItem/6/2": ("LineItem/6/2", "l_order", "Order/6"),
            },
            data,
        )
        self.assertEqual(
