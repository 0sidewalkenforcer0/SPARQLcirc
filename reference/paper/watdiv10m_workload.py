...
The generator writes a versioned external workload from the official WatDiv
suite and the checked-in O/M/P template inputs. File identity is checked by
complete byte equality where needed; the module does not compute checksums or
...
MINUS_TEMPLATES = tuple("M%d" % value for value in range(1, 6))
NON_PATH_TEMPLATES = OFFICIAL_TEMPLATES + OPTIONAL_TEMPLATES + MINUS_TEMPLATES
BOUND_PATH_TEMPLATES = ("P-plus", "P-star", "P-alt")
ALL_PAIRS_PATH_TEMPLATE = "P-plus-all"
EXPECTED_NON_PATH = 300
EXPECTED_PATH = 31
    ...
    for row in rows:
        ...
        if row["template"] in BOUND_PATH_TEMPLATES:
            ...
            if row["template"] in OFFICIAL_TEMPLATES:
                expected_source_kind = "watdiv-official-template"
            elif row["template"] in OPTIONAL_TEMPLATES:
                expected_source_kind = "reconstructed-optional-extension"
            else:
                expected_source_kind = "project-minus-extension"
            if (
...
def freeze_workload(
    *,
    ...
    """Generate a new immutable 331-query batch without touching old assets."""
    ...
    for template in OPTIONAL_TEMPLATES + MINUS_TEMPLATES:
    ...
    for template in NON_PATH_TEMPLATES:
        ...
        if template in OFFICIAL_TEMPLATES:
            source_kind = "watdiv-official-template"
        elif template in OPTIONAL_TEMPLATES:
            source_kind = "reconstructed-optional-extension"
        else:
            source_kind = "project-minus-extension"
    ...
    metadata = {
        ...
        "counts": {
            ...
            "minus_extension_templates": len(MINUS_TEMPLATES),
            "path_templates": 4,
            "non_path_queries": EXPECTED_NON_PATH,
            "path_queries": EXPECTED_PATH,
            "queries": EXPECTED_TOTAL,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    generate.add_argument(
        "--path-sources",
        type=Path,
        default=DEFAULT_PATH_SOURCES,
        help="frozen property-path source table (default: repository formal workload)",
    )
