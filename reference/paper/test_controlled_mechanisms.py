...
class ControlledMechanismsTests(unittest.TestCase):
    ...
    def test_hermetic_fake_d4_success_path_uses_fixed_argv_and_wmc(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            fake.write_text(
                ...
                "cnf_text = cnf.read_text(encoding='utf-8')\n"
                "assert 'p cnf ' in cnf_text\n"
                "assert '-m' not in args and '--dump-ddnnf' not in args, args\n"
                "weights = {}\n"
                "for line in cnf_text.splitlines():\n"
                "    parts = line.split()\n"
                "    if len(parts) == 6 and parts[:3] == ['c', 'p', 'weight']:\n"
                "        weights[int(parts[3])] = float(parts[4])\n"
                "aux = sorted(v for v in weights if v > 0 and "
                "weights[v] == weights.get(-v) == 1.0)\n"
                "edge = ' '.join(str(v) for v in [1] + aux)\n"
    ...
    def test_legacy_d4_pipeline_is_explicit_and_ci_checks_artifact_pqe(self):
        self.assertEqual(
        ...
        self.assertIn("python reference/paper/test_pqe_from_artifact.py", workflow)
        self.assertIn("python reference/verify_ddnnf_wmc.py", workflow)
