...
import sys
from ddnnf_wmc import evaluate_text, evaluate_tseitin_text, NNFError
...
# d4v2 omits branch-local free variables. Variable 3 is a (1,1) Tseitin
# auxiliary: it is fixed on the first branch and free on the second. The two
# root arcs model d4v2's unused internal variable zero.
D4_TSEITIN_FREE = """o 1 0
o 2 0
t 3 0
1 2 0 0
1 2 -0 0
...
def main():
    ...
    tseitin = evaluate_tseitin_text(
        D4_TSEITIN_FREE, {1: P[1], 2: P[2], 3: (1.0, 1.0)}
    )
    auxiliary_count = 5000
    wide = evaluate_tseitin_text(
        "o 1 0\nt 2 0\n1 2 "
        + " ".join(str(variable) for variable in range(1, auxiliary_count + 1))
        + " 0\n",
        {variable: (1.0, 1.0) for variable in range(1, auxiliary_count + 1)},
    )
    ...
    ok = (ok and abs(tseitin.probability - 0.68) < 1e-12 and
          abs(wide.probability - 1.0) < 1e-12)
    ...
    print(f"[d4 Tseitin] {tseitin.probability:.6f} expected=0.680000")
    print(f"[scaled {auxiliary_count}] {wide.probability:.6f} expected=1.000000")
