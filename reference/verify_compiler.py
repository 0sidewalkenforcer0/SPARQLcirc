...
def _check_wide_monotone_cudd():
    """Wide source gates must compile as a linear ordered BDD chain."""
    width = 4096
    circ = {}
    weights = {}
    leaves = []
    for index in range(width):
        leaf = "wide-leaf-%04d" % index
        token = "urn:test:wide:%04d" % index
        circ[leaf] = ("leaf", token)
        weights[token] = 0.5
        leaves.append(leaf)
    circ["wide-or"] = ("plus", tuple(leaves))
    circ["wide-and"] = ("times", tuple(leaves))
    roots = {"wide-or": "wide-or", "wide-and": "wide-and"}
    batch = compiler.compile_many(circ, roots, backend="cudd")
    values = batch.wmc_many(weights)
    _close(values["wide-and"], 0.0, "wide CUDD conjunction underflow")
    _close(values["wide-or"], 1.0, "wide CUDD disjunction saturation")
    sizes = batch.root_sizes()
    if sizes != {"wide-and": width, "wide-or": width}:
        raise AssertionError("wide monotone gates are not linear BDDs: %r" % sizes)
