...
def _parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        choices=("Standard", "SPARQL_Star", "SPARQL_Star_Row",
                 "Standard_Pure", "SPARQL_Star_Pure"),
        help=("mixed layout by default; SPARQL_Star_Row assigns one token to a complete "
              "row-shaped subject; *_Pure is the historical token-only compatibility mode"),
    )
    ...
    p.add_argument("--construction", choices=("factorised", "flat"),
                   help="construction mode passed to the engine (default: the engine's own "
                        "default, factorised); flat is the read-only-endpoint route")
...
def evaluate(nt: str, probabilities: dict[str, float], compile_mode: str = "shared",
    ...
    def answer_key_for_root(root: str) -> str:
        ...
        binding_by_key.setdefault(binding_key, bindings[root])
        return binding_key

    roots, answer_root_normalization = circuit_io.merge_answer_roots(
        circ, answers, answer_key_for_root
    )
    ...
    rows = []
    for binding_key, root in sorted(roots.items()):
        binding = binding_by_key[binding_key]
        rows.append({
            "binding": {name: _term_json(value) for name, value in sorted(binding.items())},
