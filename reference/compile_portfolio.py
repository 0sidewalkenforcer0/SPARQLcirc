...
Motivation: ProvSQL computes exact probability with a cost-ranked exact
portfolio. This module provides a portable subset for SPARQLcirc artifacts,
but is not claimed to reproduce ProvSQL's tree-decomposition or selection
logic. Formal d4 measurements use :mod:`d4_from_artifact`, which calls the
pinned compiler directly for every answer root.
...
def d4_compile_once(circ, root, P, d4bin=None, timeout=COMPILE_TIMEOUT_S):
    ...
    with tempfile.TemporaryDirectory(prefix="portf_") as d:
        ...
        t = time.perf_counter()
        ev = ddnnf_wmc.evaluate_tseitin_file(nnf, e["weights"])
