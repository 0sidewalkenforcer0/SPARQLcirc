...
def merge_answer_roots(circ, answers, key_for_root):
    """Return one root per observable answer, adding Plus gates when needed.

    An endpoint may use distinct internal numeric terms to construct answer
    gate IRIs and later serialize those terms to the same canonical lexical
    form.  The resulting gates denote alternative derivations of one
    observable answer.  Keep every gate and join each colliding group under a
    deterministic synthetic Plus root.

    ``circ`` is updated in place so existing compilers can consume the returned
    root mapping directly.
    """
    grouped = {}
    for root in sorted(answers):
        grouped.setdefault(key_for_root(root), []).append(root)
    ...
    for key in sorted(grouped):
        children = tuple(sorted(grouped[key]))
        if len(children) == 1:
            roots[key] = children[0]
            continue
        while True:
            candidate = "urn:circuit:canonical-answer-merge:%d" % synthetic_index
            synthetic_index += 1
            if candidate not in circ:
                break
        circ[candidate] = ("plus", children)
        roots[key] = candidate
        merge_nodes += 1
        merge_edges += len(children)

    return roots, {
        "raw_answer_root_count": len(answers),
        "canonical_answer_count": len(roots),
        "duplicate_answer_keys": merge_nodes,
        "duplicate_answer_roots": len(answers) - len(roots),
        "merge_plus_nodes": merge_nodes,
        "merge_plus_edges": merge_edges,
    }
...
def answer_probs(nt, P, prob_fn):
    ...
    roots, _merge_metrics = merge_answer_roots(
        circ, answers, lambda gate: answer_key(bindings[gate])
    )
    out = {}
    for key, g in roots.items():
        r = prob_fn(circ, g, P)
        out[key] = round(r[0] if isinstance(r, tuple) else r, 10)
    return out
